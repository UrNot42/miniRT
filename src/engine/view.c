...
/*   Updated: 2024/03/25 17:53:30 by ulevallo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_cam	camera(t_unt hsize, t_unt vsize, float field_of_view)
{
	...
	cam.hz_size = hsize;
	cam.v_size = vsize;
	cam.o.fov = field_of_view;
	set_transform(&cam.o, get_id4mtrx());
...
t_ray	ray_for_pixel(t_cam view, t_unt	px, t_unt py)
{
	...
	pixel = tup_mtrx(inverse_4mtrx(view.o.transform),
			set_point(world.x, world.y, -1));
	r_origin = tup_mtrx(inverse_4mtrx(view.o.transform), origin());
